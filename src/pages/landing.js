...
function initMeshAnimation() {
  ...
  function generatePoints() {
    ...
    for (let j = 0; j <= rows; j++) {
      for (let i = 0; i <= cols; i++) {
        ...
        points.push({ ox: px, oy: py, x: px, y: py, vx: (Math.random() - 0.5) * 0.18, vy: (Math.random() - 0.5) * 0.08, size: Math.random() < 0.07 ? 4 : 2, cols, rows, i, j });
      }
    }
  }
  ...
  function draw(t) {
    ...
    for (let j = 0; j <= rows; j++) {
      for (let i = 0; i <= cols; i++) {
        ...
        if (i < cols) {
          ...
          ctx.strokeStyle = `rgba(0, 220, 240, ${alpha})`; ctx.lineWidth = 0.8; ctx.stroke();
        }
        if (j < rows) {
    ...
    for (const p of points) {
      ...
      if (glow) {
        ...
        grad.addColorStop(0, 'rgba(0,242,255,0.9)'); grad.addColorStop(1, 'rgba(0,242,255,0)');
        ctx.beginPath(); ctx.arc(p.x, p.y, 8, 0, Math.PI * 2); ctx.fillStyle = grad; ctx.fill();
      }
      ctx.beginPath(); ctx.arc(p.x, p.y, glow ? 3 : 1.5, 0, Math.PI * 2);
      ctx.fillStyle = glow ? '#00f2ff' : 'rgba(0,210,230,0.7)'; ctx.fill();
  ...
  setSize(); generatePoints(); animId = requestAnimationFrame(draw);
  const obs = new ResizeObserver(() => { cancelAnimationFrame(animId); setSize(); generatePoints(); animId = requestAnimationFrame(draw); });
  obs.observe(canvas);
}
